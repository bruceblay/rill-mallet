...
static void checkTempoChange() {
  for (uint32_t seed : {2u, 3u}) {
    ...
    for (int64_t i = 1; i <= int64_t(mallet::rate) * 50; ++i) {
      ...
      if (switchAt >= 0) {
        if (i < switchAt + int64_t(mallet::rate) * 8) during = std::max<int64_t>(during, std::llabs(error));
        else worst = std::max<int64_t>(worst, std::llabs(error));
      }
    }
    assert(engine.bpm() == slower);
    assert(worst < mallet::rate * 2 / 1000);
...
int main() {
  ...
  for (uint32_t seed : {1u, 7u, 0x6c696665u}) {
    ...
    const int64_t span = engine.barSamples(), check = mallet::rate * 120 / 1000;
    const int64_t origin = span / 3;  // where the shared beat falls
    int64_t trimmed = 0, worst = 0;
    for (int64_t i = 1; i <= int64_t(mallet::rate) * 60; ++i) {
      engine.sample();
      if (i > int64_t(mallet::rate) * 15 && i % (mallet::rate * 3) == 0) engine.newVariation();
      if (i % check) continue;
      int64_t want = ((int64_t(engine.frames()) - origin) % span + span) % span;
      int64_t error = ((want - int64_t(engine.barPhase())) % span + span) % span;
      if (error > span / 2) error -= span;
      engine.trimGrid(int32_t(error / 4));
      if (i > int64_t(mallet::rate) * 15) { trimmed += std::llabs(error / 4); worst = std::max<int64_t>(worst, std::llabs(error)); }
    }
