...
// Rill's score, playing multisampled tuned percussion instead of synthesis.
//
// The composer below is Rill's, unchanged: the same phrase generation,
// development, answers, harmony and delay. Only the voice layer differs. A
// note picks the nearest zone of the current instrument and resamples from
// that zone's root pitch, so nothing is ever bent more than a couple of
// semitones and the character of the recording survives the transposition.
//
// The clips live in flash and are read a sample at a time; none of them is
...
class Engine {
  struct Voice {
    ...
    uint32_t length = 0;
    float position = 0, step = 1, gain = 0;
  };
  ...
  // down a little at each tick rather than applied at once. A jumped tick is
  // a dropped or doubled note, which is louder than being a few milliseconds
  // out of line.
  int32_t gridTrim = 0;
  ...
  void note(int midi, float seconds, float attack, float release, float gain, float color) {
    for (auto& v : voices) if (v.duration == 0) {
      ...
      // Nearest zone, then resample from its root. Stretching one clip across
      // the whole range is what makes a sampled instrument sound like a
      // cartoon at its edges; a zone every few semitones keeps every note
      // within a tone or so of something real.
      unsigned choice = 0;
      int nearest = 128;
      for (unsigned z = 0; z < set.count; ++z) {
        int distance = std::abs(int(set.zones[z].root) - midi);
        if (distance < nearest) { nearest = distance; choice = z; }
      }
      const samples::Zone& zone = set.zones[choice];
      v.data = zone.data;
      v.length = zone.length;
      v.step = std::pow(2.0f, float(midi - int(zone.root)) / 12.0f);
 ...
 public:
  ...
  // Where this engine sits inside a bar of four beats, in samples. Its
  // phrases are not four bars long and are not meant to be; what the
  // ensemble shares is the pulse under them.
  ...
  float sample() {
    ...
    for (auto& v : voices) if (v.duration) {
      ...
      // Linear interpolation between neighbouring frames. At the resampling
      // ratios a zone every four semitones asks for, at most a tone and a
      // half either way, there is nothing here worth a better interpolator.
      float tone = 0;
      uint32_t index = uint32_t(v.position);
      if (index + 1 < v.length) {
        float fraction = v.position - float(index);
        float a = float(v.data[index]), b = float(v.data[index + 1]);
        tone = (a + (b - a) * fraction) * (1.0f / 32768.0f);
      }
